<template>
	<view class='search-wrapper'>
		<view class='search-header topbar'>
			<input class='search-input' placeholder-class='phcolor' :placeholder='placeholder' v-model='value'></input>
		...
		<view class='result-list'>
			<view v-if='movies.length' class='movie-box'>
				...
				<navigator v-for='(movie,index) in movies' :key='movie.id' class='movie-section' url=`/pages/movie-detail/movie-detail?movieId=${movie.id}`>
			...
			<view v-if='cinemas.length'>
				...
				<cinemaSection v-for='(cinema,item) in cinemas' :key='cinema.id' :cinema='cinema' />
			</view>
		</view>
		<view v-show='!isEmpty'>
...
<script>
	...
	export default {
		...
		watch:{
			value:function(val){
				let value = val
				this.$request(`/ajax/search?kw=${value}&cityId=57&stype=${this.stype}`).then(res => {
					let movies = res[1].data.movies ? res.data[1].movies.list : []
					movies = movies.map(item => {
						item.img = item.img.replace('w.h', '128.180')
						return item
					})
					this.movies = movies
					this.cinemas = res[1].data.cinemas ? res[1].data.cinemas.list : []
					this.isEmpty = this.movies.length || this.cinemas.length || !this.value
				})
